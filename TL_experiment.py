...
def TL_experiment():
    ...
    for i in range(nums_experiments):
        ...
        # train
        source_r2, source_RMSE, source_label_loss = train(source_feature=train_source_dataset_name,
                                                          source_label=train_source_label_name,
                                                          target_feature=train_target_dataset_name,
                                                          target_label=train_target_label_name)
        
        # test
        target_r2, target_RMSE, target_label_loss = test(source_feature=test_source_dataset_name,
        ...
        with open('process_data/results7.csv', mode='a') as results_file:
            results_writer = csv.writer(results_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            results_writer.writerow(['source_dataset_name', 'target_dataset_name', 'source_r2', 'source_RMSE', 'source_label_loss', 'target_r2', 'target_RMSE', 'target_label_loss'])
            results_writer.writerow([remove(train_source_dataset_name), remove(train_target_dataset_name), source_r2, source_RMSE, source_label_loss, target_r2, target_RMSE, target_label_loss])
