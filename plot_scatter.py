...
def plot_scatter(source_feature, source_label, target_feature, target_label, model_name, folder_name, task_number, source_kla, source_mu, target_kla, target_mu):
    batch_size = 1344
    alpha = 0
    ...
    my_net = torch.load(os.path.join('saved_models', model_name + source_dataset_name + '_' + target_dataset_name + '.pth'))
    ...
    # create the subfolder in figures folder
    if not os.path.exists(os.path.join('figures', folder_name)):
        os.makedirs(os.path.join('figures', folder_name))
    # save the plot with souce and target dataset names
    plt.savefig(os.path.join('figures', folder_name, 'scatter_plot_' + source_dataset_name + '_' + target_dataset_name + '.png'))
    plt.show()

if __name__ == "__main__":
    models_list = ['DANN_model_', 'DANN5_model_', 'DANN7_model_']
    folder_list = ['DANN', 'DANN5', 'DANN7']
